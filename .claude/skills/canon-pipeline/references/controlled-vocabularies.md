...
Every enum field in the CANON schema must use one of these exact values. Do not paraphrase, translate inconsistently, or invent new values — the viewer and any downstream tooling match on these strings exactly.
...
### Período Histórico (controlled vocabulary for contextoHistorico)
OT: `"Prehistórico / Primordial"` | `"Bronce Temprano (3300–2000 a.C.)"` | `"Bronce Medio (2000–1550 a.C.)"` | `"Bronce Tardío (1550–1200 a.C.)"` | `"Hierro I (1200–1000 a.C.)"` | `"Hierro II (1000–586 a.C.)"` | `"Período Persa (539–332 a.C.)"` | `"Período Helenístico (332–63 a.C.)"`
NT: `"Período Romano Temprano (63 a.C.–70 d.C.)"` | `"Período Romano Tardío (70–100 d.C.)"`
...
## Nivel de fuente (fuentes[].nivel)
`1` (primary study Bible / confessional standard) | `2` (systematic/biblical theology, historical scholarship) | `3` (secondary/tertiary reference)

## Idioma de fuente (fuentes[].idioma)
`"es"` | `"en"` | `"pt"` | `"todos"`

## Rol de revisor (meta.revisadoPor.rol)
`"teólogo"` | `"editor"` | `"administrador"`

## Nombre de fase del pipeline (meta.pipeline.historial[].nombre)
`"diseño_esquema"` | `"registro_fuentes"` | `"extraccion"` | `"validacion"` | `"renderizado"`
...
## Canonical verse reference abbreviations (use per language)

All inline verse references in text fields **must** use the canonical abbreviation for their language. The viewer's detection regex requires this — unlisted forms will not generate links.

**Format rules (all languages):**
- Range: en-dash `–` → `Nm 6:24–26`
- Multi-verse: comma → `Lc 24:27,44`
- Chapter-only / chapter-range: omit colon+verse when citing a whole chapter or span → `Nm 16`, `Nm 22–24`
- `ref` key fields (language-neutral): always use the **ES** abbreviation
- Every reference in a semicolon list must be fully qualified — never omit the book prefix after `;` → `Nm 1; Nm 26`, not `Nm 1; 26`

### ES — Spanish (NBLA)
`Gn` `Ex` `Lv` `Nm` `Dt` `Jos` `Jue` `Rt` `1S` `2S` `1R` `2R` `1Cr` `2Cr` `Esd` `Neh` `Est` `Job` `Sal` `Pr` `Ec` `Cnt` `Is` `Jer` `Lm` `Ez` `Dn` `Os` `Jl` `Am` `Abd` `Jon` `Miq` `Nah` `Hab` `Sof` `Hag` `Zac` `Mal` · `Mt` `Mc` `Lc` `Jn` `Hch` `Ro` `1Co` `2Co` `Gá` `Ef` `Fil` `Col` `1Ts` `2Ts` `1Ti` `2Ti` `Tit` `Flm` `He` `Stg` `1P` `2P` `1Jn` `2Jn` `3Jn` `Jud` `Ap`
